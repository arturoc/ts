...
export class NodeLoader {
    ...
    parseNode(buffer: ArrayBuffer, id: string, deviceProfile: DeviceProfile, version: string, useWasmParser: boolean): Promise<NodePayload | undefined> {
        const { payloadPromises } = this;
        const enableOutlines = deviceProfile.features.outline;
        const applyFilter = true;
        const parseMsg: ParseMessage = { kind: "parse", buffer, id, version, separatePositionsBuffer: true, enableOutlines, applyFilter, useWasmParser };
    ...
    loadNode(node: OctreeNode, version: string, useWasmParser: boolean): Promise<NodePayload | undefined> {
        const { payloadPromises } = this;
        ...
        const enableOutlines = deviceProfile.features.outline;
        const applyFilter = true;
        const loadMsg: LoadMessage = { kind: "load", id, version, url: url.toString(), byteSize, separatePositionsBuffer: true, enableOutlines, applyFilter, useWasmParser };
